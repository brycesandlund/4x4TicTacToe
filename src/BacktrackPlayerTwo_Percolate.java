 ...
 *
 */
public class BacktrackPlayerTwo_Percolate {
	...
	private final int[] usedPositions = new int[16];
...
//	public static final int symmetryDepth = 17;
	...
	public static void main(String[] args) {
		BacktrackPlayerTwo_Percolate backtrack = new BacktrackPlayerTwo_Percolate();
		System.out.println(backtrack.backtrack(true, 0));
	}
	
	public BacktrackPlayerTwo_Percolate(byte[] start1Nums, byte[] start2Nums)
	{
	 ...
	 */
	public BacktrackPlayerTwo_Percolate(List<Byte> grid)
	{
		initializeClosed();
	 ...
	 */
	public BacktrackPlayerTwo_Percolate()
	{
		initializeClosed();
		setup();
	}
...
//		}
	...
	private void percolate(List<Byte> grid, int depth, boolean result)
	{
		for (int i = 0; i < depth; ++i)
		{
			byte temp = grid.get(usedPositions[i]);
			grid.set(usedPositions[i], (byte)0);
			List<Byte> symmetry = canoc.canonicalize(grid, depth <= symmetryDepth);
			storeResult(symmetry, result, depth);
			grid.set(usedPositions[i], temp);
		}
	}
