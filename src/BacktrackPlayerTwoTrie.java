 ...
 *
 */
public class BacktrackPlayerTwoTrie {
...
//	public static final int symmetryDepth = 17;
    public static final int storeDepth = 0;//10;//9;
	
	public static final int symmetryDepth = 0;//5;//5;
	...
	public static void main(String[] args) {
		BacktrackPlayerTwoTrie backtrack = new BacktrackPlayerTwoTrie();
		System.out.println(backtrack.backtrack(true, 0));
	}
	
	public BacktrackPlayerTwoTrie(byte[] start1Nums, byte[] start2Nums)
	{
	 ...
	 */
	public BacktrackPlayerTwoTrie(List<Byte> grid)
	{
		initializeClosed();
...
//		}
	...
	//	if (CLOSED.size() < 55000000)	//should be able to put 80 (55 with depth keys) million elements into the hashmap with 7GB of memory
		...
		//clear old cached objects as they are less likely to be of importance and memory is running out
		if (closedSize >= 90000000)//90000000)
		{
			long elementsPurged = 0;
			for (int i = symmetryDepth + 1; i <= storeDepth; ++i)
			{
				closedSize -= CLOSED[i].size();
				elementsPurged += CLOSED[i].size();
				CLOSED[i].clear();
			}
			System.out.println(elementsPurged + " elements purged.");
		}
	...
	private void printStats()
	{
		for (int i = 0; i < 17; ++i)
		{
			System.out.printf("Depth %2d: Times Visited: %12d, Times Cached: %9d, Elements Stored: %11d, Times Player2 Won: %10d\n", i, timesVisited[i], timesCached[i], i == 16 ? 0 : CLOSED[i].size(), timesPly2Won[i]);
		}
	}
