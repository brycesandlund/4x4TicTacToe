...
public class BacktrackLongs {
	 ...
	 */
	private final HashMap<Long, Boolean>[] CLOSED = new HashMap[16];
...
//	public static final int storeDepth = 17; //store everything mothafucka
	...
	public BacktrackLongs(byte[] start1Nums, byte[] start2Nums)
	{
	 ...
	 */
	public BacktrackLongs(byte[] grid)
	{
		initializeClosed();
	...
	private void initializeClosed()
	{
		...
		{
			CLOSED[i] = new HashMap<Long, Boolean>();
		}
	}
	...
	public boolean backtrack(boolean play1Turn, int depth)
	{
		...
		boolean[] result = getSymmetricResult(grid, depth);
		if (result[0] == true)
		{
			++timesCached[depth];
			if (result[1])
...
//		}
	...
	public void storeResult(byte[] grid, boolean result, int depth)
	{
		...
		++closedSize;
		CLOSED[depth].put(mapping, result);
	}
	 ...
	 */
	private boolean[] getSymmetricResult(byte[] grid, int depth)
	{
		long mapping = canoc.canonicalize(grid);
		if (CLOSED[depth].containsKey(mapping))
		{
			return new boolean[]{true, CLOSED[depth].get(mapping)};
		}
