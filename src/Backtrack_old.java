...
public class Backtrack_old {
	 ...
	 */
	LinkedList<Integer> ply1Nums;
	
	/**
	 * Player 2's possible numbers. Data structure TBD. I think a BST might be better.
	 */
	LinkedList<Integer> ply2Nums;
	
	public static void main(String[] args) {
		Backtrack_old blah = new Backtrack_old();
	...
	public boolean backtrack(boolean ply1Turn)
	{
		...
		while (nextLocations.hasNext())
		{
			...
			//keep track of each used numbers so we can use them again on different locations
			LinkedList<Integer> usedNums = new LinkedList<Integer>();
			
			//try all possible placement numbers at this location
			while (!currentNumbers.isEmpty())
			{
				int nextNum = currentNumbers.pollFirst();
			...
			//restore the status of currentNumbers to what it was pre-while loop
			currentNumbers.addAll(usedNums);
		}
	 ...
	 */
	public boolean hasWinningMove(int[][] grid, LinkedList<Integer> nums)
	{
		return false;
	}
	 ...
	 */
	public Iterator<Point> getNextMoves(int[][] grid, LinkedList<Integer> nums)
	{
